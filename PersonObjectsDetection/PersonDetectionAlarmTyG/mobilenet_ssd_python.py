...
async def main():
    ...
    while True:
        ...
        if not message_sent and person_detected:
            ...
            await send_telegram_message(bot,chat_id,message_text, image_base64)
            message_sent = True  # Set the flag to True after sending the email
            last_message_time = time.time()  # Update the last sent time

        # Check if the maximum time without detection has passed
        if message_sent and (time.time() - last_message_time > max_time_without_detection):
